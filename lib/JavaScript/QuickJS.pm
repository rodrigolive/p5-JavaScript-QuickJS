...
=head2 $bytecode_func = I<OBJ>->compile( $JS_CODE )

Compiles JavaScript code to bytecode without executing it. Returns a
L<JavaScript::QuickJS::Function> object representing the compiled bytecode.

This is useful when you want to compile code once and execute it multiple times,
or when you want to separate compilation from execution.

    my $js = JavaScript::QuickJS->new();
    my $compiled = $js->compile('(x) => x * 2');

    # The code is compiled but not executed yet
    # You can serialize it:
    my $bytecode = $compiled->to_bytecode();

Note: The returned function object represents compiled bytecode. To execute it,
call it like a normal function or use C<from_bytecode()> after serializing.

=head2 $func = I<OBJ>->from_bytecode( $BYTECODE )

Deserializes and restores a function from QuickJS bytecode. The bytecode should
be a binary string previously generated by C<to_bytecode()>.

Returns a L<JavaScript::QuickJS::Function> object that can be called like any
other JavaScript function.

    # Session 1: Create and serialize a function
    my $js1 = JavaScript::QuickJS->new();
    my $func = $js1->eval('(x) => x * 2');
    my $bytecode = $func->to_bytecode();

    # Save bytecode to database/file/etc.
    save_to_storage($bytecode);

    # Session 2: Restore and use the function
    my $js2 = JavaScript::QuickJS->new();
    my $loaded_bytecode = load_from_storage();
    my $restored = $js2->from_bytecode($loaded_bytecode);
...
=item * Storing event handlers or callbacks in a database for later execution

=item * Pre-compiling JavaScript code during build time and loading it at runtime

=item * Transferring functions between different JavaScript contexts or processes

=item * Implementing persistent JavaScript functions that survive application restarts
...
=item * The bytecode must be treated as a byte string (binary data), not a character string.

=item * Functions with closures over external variables may not work correctly unless
those variables are available in the new context.

=item * The new JavaScript context is completely independent - it doesn't share
global variables or state with the original context.
